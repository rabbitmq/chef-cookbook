...
property :cluster_nodes, String, name_property: true # first node name to join
property :cluster_name, String                       # cluster name
...
action :change_cluster_node_type do
  ...
  var_cluster_node_type = parse_cluster_nodes_string(new_resource.cluster_nodes).compact.select { |node| node['name'] == var_node_name }.first['type']
