...
describe 'rabbitmq::default' do
  let(:runner) do
    ChefSpec::ServerRunner.new(REDHAT_OPTS) do |node, _|
      node.override['rabbitmq']['version'] = '3.7.15'
    end
  end
  ...
  describe 'ubuntu' do
    ...
    it 'creates a rabbitmq-server deb in the cache path' do
      expect(chef_run).to create_remote_file_if_missing('/tmp/rabbitmq-server_3.7.15-1_all.deb')
    end
  ...
  describe 'redhat' do
    ...
    let(:rpm_file) { 'rabbitmq-server-3.7.15-1.el7.noarch.rpm' }
